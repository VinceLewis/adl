...
Before tasks that change `src/server/migrations/roles.sql`,
`src/server/migrations/grants.sql`, any ordered migration file, the database
role split, the runbook's migration procedure, or `scripts/dev/postgres.sh`,
also read:
...
`src/reference/giggle-band/domain.adl`/`ui.adl` no longer exist: they were a
frozen model-version-1.0.0 snapshot that had drifted from the `.adlj` source in
32 recorded ways, could not be regenerated from it, and were deleted in
Phase 98 — see `implementation/reference-app-drift.md` before keeping any
superseded file on disk "for reference".
...
controls, mobile business-context selectors, or anything that runs a `COMMAND`
from shell chrome (a `COMMAND_ACTION` control, the `EMPTY_STATE` placement, the
`CONTEXT ... UNAVAILABLE` visibility predicate, or `<adl-command-form>`), also
read:

- `implementation/shell-navigation.md`
- `implementation/command-intent-replay.md`

Before tasks that change whether a person may register at all — the
`REGISTRATION` app declaration, `resolveSelfServiceRegistration`, the anonymous
registration ceremony, its rate bucket, or the first-run onboarding surface —
also read:
...
Before keeping any superseded or generated file on disk "for reference",
before adding a line-number citation into a file this repository maintains, and
before representing one application twice on disk in two encodings, read:
...
- `process/testing-expectations.md`: read before code implementation or verification work. It carries the project-agnostic testing doctrine, written so it can be lifted into a shared kit: **every positive test needs at least one matching negative test** and missing negatives are written first; **the mutation check** — break the implementation on purpose and confirm the test goes red, with the two ways a test proves nothing (nothing exercises it; a different check is satisfying it), each caught here by mutation and not by review; **an absence assertion needs a present-anchor**, since "X is not there" is satisfied by nothing being there at all; and **expected-failure lists are built from a report-only inventory run, never from prediction** (four of four predictions were wrong when measured). Plus what the negative half looks like per subsystem (policy, diagnostics, grants, lifecycle, commands, browser specs). Since Phase 96 it also carries the rule that changing a shipped reference app's object constraints, field requiredness or validators invalidates `tests/integration/` fixtures built from that model, which `npm test` never runs.
- `process/instruction-placement.md`: read before adding a rule to `AGENTS.md`, `CLAUDE.md` or `learnings/`, or before trimming either file. It carries the test that decides inline-versus-reference — does the reader know they are in the situation the rule covers? — the five-rule cap on `CLAUDE.md`'s Testing section, and the rule that a mechanical enforcement (hook, lint, diagnostic) beats any prose placement.
...
- `implementation/context-ui-navigation.md`: read before changing context selectors, view navigation, context-aware dashboard rendering, or browser UI calls for context-scoped objects. Since Phase 105 it also carries what a `CONTEXT ALL` view resolves onto its runtime context, and that such a view short-circuits to the shell's own empty state before its list's declared one when the caller can reach no instance.
...
- `implementation/policy-engine.md`: read before changing policy evaluation, policy conditions, runtime record returns, UI policy presentation, lifecycle action visibility, or tests that assert policy-shaped output -- it carries the Jointly Care reference app's discovery that a `WHEN`-conditioned `SEARCH`/`EXPORT` rule can never match (no candidate record exists at the coarse object-level gate) and that a `ROLE` condition on an object with no `SCOPE` can only ever match a role earned through that same object's own context, never a role earned through an unrelated one — plus the Phase 91 finding that documenting that trap did not stop it: Giggle Band shipped the identical dead `ROLE BandMember` rule on `User`, so no band member could read or search any user and every lookup label in the app degraded to a raw id, which Phase 93 turned into a compile error (`ADL_POLICY_ROLE_PRINCIPAL_UNREACHABLE`) whose limits — what it proves and what it stays silent about — are recorded there. **Phase 101** then added the other half: a field-scoped `ALLOW READ ... FIELDS <field>` grants that field without granting the record (two conformance cases pin it), which is how both reference apps' `UserPolicy` now lets any signed-in caller resolve a user's display name while refusing the record, the `Email` field and the directory — and why a lookup label must be issued as a *field* read (`applyDisplayFieldReadPolicy`, `readFieldsForDisplay`) rather than through `applyReadPolicy`'s row gate, which would degrade every label in the app to a raw `user-...` id with a clean typecheck and a green suite. **Phase 103** adds the `SELF` principal (`{ "match": "self" }`, `record.meta.guid === context.userId`): what `OWNER` cannot say about a `User` record, why `ALLOW SEARCH SELF` is a compile error, why the recordless search gate is what keeps a self-grant from reopening a user directory, and why a search-refusal test is vacuous unless the rule under test actually names `SEARCH`.
...
- `implementation/reference-app-drift.md`: read before keeping any superseded or generated file on disk "for reference", before adding a line-number citation into any maintained file, and before representing one application twice on disk in two encodings -- it carries the Phase 94 finding that Giggle Band's kept `domain.adl`/`ui.adl` were a frozen model-version-1.0.0 snapshot rather than a view of `domain.adlj`/`ui.adlj` (32 enumerated divergences) that *could not* be regenerated because the real source uses three constructs with no ADL text syntax at all (`conflictOverlay`, `projectedFields`, `summary`), and the Phase 98 decision to delete them rather than keep pinning the drift -- plus why the eliminating move beats the pin, why `.adl`-as-printout makes a checked-in `.adl` corpus a liability and what to reach for instead, the measured lesson that a divergence pin over *paths* is not a pin over *values* (renaming an already-divergent nav label left the path set identical and the test green), the two diff mechanics that made a 1,629-line comparison readable as 32 entries (re-key named arrays by name; report the shallowest differing path and stop), the evidence that a line number is a durable pointer and still a misleading citation (20/20 resolved, and all twenty described a release nobody was running), and the two printer defects a frozen fixture had been hiding (`MIGRATION` never printed at all; a policy clause list swallowing a following `FIELDS`) -- and note that Phase 100 has since given all three blocking constructs text syntax, so both reference apps round-trip again; the lesson is the diagnosis, not the standing refusal.
...
- `implementation/production-authority-operations.md`: read before changing authority HTTP deployment, cookies, upstream identity proof, rate limits, credential logging, PostgreSQL operations, database roles or grants, or incident procedures -- since Phase 102 it carries the measured two-role grant gap (a runbook-following deployment connected and then failed every query with `permission denied for table adl_authority_*`), why the fix is a third file `grants.sql` run as `adl_migrator` rather than `alter default privileges for role` in `roles.sql` (which a non-superuser `CREATEROLE` database owner is refused), and why no test using the shared integration harness can see any of it.
...
- `implementation/presentation-matrix-runtime.md`: read before changing resource/date matrices, availability presentation, matrix cell cycling, range editing, or future calendar work that reuses matrix status semantics. Since Phase 104 it also carries `MATRIX`'s ADL text syntax (`ROWS`/`COLUMNS`/`CELLS`/`CELL`/`EDIT`), why `CELL` is a block of its own (`cell.status` overrides `cellSource.status` and a flat `STATUS` cannot say which it means), why `UNSET_VALUE null` and an absent `UNSET_VALUE` are different models, the `STATUS <map>()` spelling for a field-less status map candidate whose parentheses are the only thing keeping it from reparsing as a direct status, and why the conformance corpus rather than a reference app is the round-trip subject.
...
- `implementation/shell-navigation.md`: read before changing shell navigation metadata, drawer rendering, top-bar controls, mobile business-context selectors, or anything that runs a `COMMAND` from shell chrome. Since Phase 99 it also carries the `COMMAND_ACTION` control kind (the one shell control that is about the application rather than the device or the session, and the only one that opens a form), why a presentation `ACTION` could not do this job, the `EMPTY_STATE` placement and why it deliberately has no region control list, the `CONTEXT ... UNAVAILABLE` visibility predicate that makes an onboarding surface self-removing, the rule that a command action renders only for a signed-in caller (the signed-out identity is a non-empty placeholder, so a bare `authenticated` create policy would accept its write and the authority would then refuse to sync it), why the shell rather than the form element must hold the form's draft, and the `refreshFromRuntime` defect that showed a newly registered person an error banner instead of an empty state. Since Phase 92 it also carries the top bar's three visual registers (readout / control / disabled control), the two contrast traps that produced them (a translucent *white* overlay on a coloured bar measures 4.08:1 for white text; `button:disabled`'s blanket opacity measures 2.03:1 on the primary bar), the mobile `align-items: stretch` label-alignment trap, the app-scoped-copy specificity trap that kept the mobile `justify-content: flex-start` rule dead from Phase 28 to Phase 95, the still-open mobile context-selector width defect, and the Phase 92 finding that a defect was investigated against a `themeSwitch` placement that a stale `.adl` snapshot showed in the top bar while `ui.adlj` had already moved it to the nav drawer (that snapshot is gone; see `implementation/reference-app-drift.md`).
- `implementation/local-https-development.md`: read before changing `vite.config.ts`'s server/preview configuration, anything under `scripts/dev/`, `playwright.config.ts`'s web servers, the `dev:*` npm scripts, or before proposing a development mode for a production check in `src/server/authority-config.ts` — it carries the Phase 97 decision to run real TLS locally rather than relax any of the three HTTPS controls, why TLS terminates in a committed dev proxy rather than in `authority-node.ts`, why Vite HTTPS is opt-in behind `ADL_DEV_HTTPS` (four Playwright web servers speak plain HTTP), why `passkey` and not `bypass` is the documented development identity mode, the `roles.sql` grant gap for tables created by `adl_migrator` (closed in Phase 102 by `src/server/migrations/grants.sql`, run as `adl_migrator`; the standing per-migration grant obligation is gone), and the headless way to drive the real browser bridge over a real proxy for a proof.
...
- `implementation/passkey-identity.md`: read before changing WebAuthn ceremonies, identity keying or identity links, the authority's identity-verification mode, the passkey sign-in surface, or invite-based identity recovery. Since Phase 99 it also carries the corrected registration invariant (registration is anonymous only where a model declares `REGISTRATION SELF_SERVICE`, the deployment has not switched it off, and the mode is `passkey` — with the anonymous case re-checked at finish so a challenge cannot outlive the configuration that allowed it), the corrected session discriminator and its four-row table, the second rate bucket for anonymous account creation and the two honest limits of it, the conditional first-admin bootstrap, and the standing defect that nothing creates a `User` *application record* for an authority-minted identity, so every such person renders as a raw `user-…` id wherever a member name belongs.
...
- `implementation/context-grants-and-relationship-access.md`: read before changing business-context availability, the object-scope gate, policy principals, read-model joins, command-established contexts, or anything that decides whether one user may see another user's records. Since Phase 103 it also carries why `SELF` is a third principal rather than a narrower `contextMember`, and the limit the two share. Since Phase 105 it also carries the rule that every consumer which de-selects a context must re-resolve **roles and grants** (the browser shell resolved roles only for four phases, which is how a shipped `Accept` button came to be refused on every click), and the measured authority-replay gap that fixing the shell does not close.
