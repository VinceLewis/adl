...
- **And their grants, since Phase 105.** `resolveContextGrants(...)` is resolved from the same de-selected base context and merged into `contextGrants` through `withContextGrants`, mirroring what `ReadModelService.resolveExecutionContext` has always done for the same `mode: "all"`. Resolving roles alone made a context reachable only through a `CONTEXT_GRANT` — a pending invitation — visible to the view and unusable by every command run from it, which is how Jointly Care shipped an `Accept` button that could never work. See `implementation/context-grants-and-relationship-access.md` for why the two must stay in separate fields, and for the authority-replay half of the same question, which this does **not** close.
- A `CONTEXT ALL` view whose caller can reach no instance never reaches its list at all: `resolveActiveViewContext` returns the shell's own `No <Context> contexts are available for this view.` empty state first, so the list's declared `emptyState` is not what a person with nothing sees. Measured while writing Phase 105's Pair G; worth knowing before designing an empty state that assumes otherwise.
