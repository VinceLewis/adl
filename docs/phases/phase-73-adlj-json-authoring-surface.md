...
Executed. Full detail in `learnings/implementation/adlj-json-authoring-surface.md`
and `docs/spec/adlj.md`. Summary:

- `AdljSourceDocument` (`src/model/adlj-source.ts`), derived from
  `Partial*Model` via `Omit`/intersection rather than hand-copied. A full
  field-by-field sweep of `resolved-model.ts` found **18** expression-bearing
  sites, not the ~8 named in this document's evidence section — including
  every `PartialCommand*StepModel.preconditions`, typed
  `PartialPolicyConditionModel[]` rather than `ResolvedExpression[]`, which
  would have silently broken command preconditions had it been missed.
- `parseExpressionSource`, exported from `parser.ts`, backed by a new public
  `parseStandaloneExpression()` method on `AdlParser`.
- `compileAdlj`/`parseAdljDocument`/`adljSourceToPartialApplicationModel`
  (`src/compiler/compile-adlj.ts`), schema-validated via `ajv` against a
  schema generated by `ts-json-schema-generator`
  (`npm run generate:adlj-schema` → `src/model/adlj-schema.json`, checked
  in). `AdljParseError` (the `.adlj` analogue of `ParseError`) carries one
  `Diagnostic` — `ADL_ADLJ_JSON_INVALID` or `ADL_ADLJ_SCHEMA_INVALID`.
- `printPartialApplicationModelAsAdl` (`src/compiler/print-adl.ts`): the full
  declarative skeleton and every expression field, deliberately
  over-parenthesizing expressions for round-trip safety over prettiness.
  Composed view presentation and edit surfaces are not printed —
  `printView` throws a named error rather than silently dropping content.
- `examples/task-tracker.adl` / `.adlj`: the standalone fixture pair.
  `compileAdlj(...).model` is `toEqual`-deep-equal to `compileAdl(...).model`
  on the hand-equivalent `.adl` source — the phase's central acceptance
  criterion — and the printer round-trip (`.adl` → partial model → print →
  reparse → resolve) resolves to the identical model too. Getting exact
  equality required two real findings about `resolveApplicationModel`, not
  bugs but pre-existing behaviour a JSON front-end does not inherit for
  free from `.adl` text's own AST-to-partial conversion: `principal.match`
  has no default inference from a non-empty `roles` list (defaults to
  `"everyone"` regardless), and an omitted `contexts`/`readModels` key
  resolves to an absent key, not `[]`. Both are documented in
  `docs/spec/adlj.md` as authoring guidance, not changed — this phase's own
  constraint ruled out any change to `resolveApplicationModel`.
- `npm test` (993), `npm run typecheck`, `npm run format:check`, and
  `npm run build` all pass. `npm run test:integration` and
  `npm run verify:push` were not run, matching this document's own
  expectation that nothing here touches the authority server, PostgreSQL,
  or rendered UI output — though `npm run build`'s bundle-size report
  surfaced a real, un-scoped-for cost: reaching `compile-adlj.ts` through
  `src/index.ts`'s barrel export pulls `ajv` and the generated schema into
  the browser bundle unconditionally (158 KB → 203 KB gzip). Recorded as a
  named follow-up in `docs/spec/adlj.md`, not fixed here.
- Two capability gaps named in this document's own Non-goals remain exactly
  that: `PartialApplicationModel`-level source merging (would unblock mixing
  `.adl`/`.adlj` sources and merging multiple `.adlj` files) and an
  `.adl` → `.adlj` importer. Neither was started.
