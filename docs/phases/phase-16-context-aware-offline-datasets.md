...
- Dataset handling that accounts for read-model source scopes introduced in Phase 15
...
- Dataset evaluation can identify records needed by read-model sources scoped to `currentUser`, `currentContext`, and `allAvailableContexts`.
...
```text
Use ADL_Codex_Implementation_Brief_v2.md, learnings/architecture/business-contexts-and-backends.md, learnings/implementation/sync-policy.md, learnings/implementation/read-model-runtime.md, docs/phases/phase-15-read-models-and-dashboards.md, and docs/phases/phase-16-context-aware-offline-datasets.md as the source of truth.
...
5. Account for Phase 15 read-model source scopes when determining records required for cross-context dashboards.
6. Add tests for dataset selection across multiple contexts.
7. Add tests proving cache-readonly and online-required behaviour still applies.
8. Document what a future remote sync service must provide without selecting PostgreSQL as a requirement.
...
11. Review what happened in this phase and update `docs/phases/phase-17-band-app-reference.md` if actual results require changed scope, constraints, deliverables, or tasks.
