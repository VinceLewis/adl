...
| Inconsistent or incomplete restore set | Metadata-only projection integrity/restore verification (`consistent`, `acceptedOutcomeRecordsMissing`, `orphanRecords`, `auditScopeInconsistent`, `membershipProjectionMissing`/`Orphaned`/`Stale`) that prints no protected JSON and blocks traffic switch when inconsistent | Phase 44/45/54 integrity tests, operations drill |
